...
getPublicIpv4(){
  local ip
  local sources=("https://api4.ipify.org?format=text" "https://checkip.amazonaws.com" "https://ipv4.icanhazip.com")
  for source in "${sources[@]}"; do
    ip=$(curl -s4m5 -k "$source" 2>/dev/null | tr -d '\r\n' | grep -m1 -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}')
    if [[ -n $ip ]]; then
      echo "$ip"
      return 0
    fi
  done
  ip=$(curl -s4m5 -k https://myip.ipip.net 2>/dev/null | sed -n 's/^当前 IP：\([^ ]*\).*/\1/p')
  if [[ -n $ip ]]; then
    echo "$ip"
    return 0
  fi
...
getPublicIpv6(){
  local ip
  local sources=("https://api6.ipify.org?format=text" "https://api64.ipify.org?format=text" "https://ipv6.icanhazip.com" "https://6.ident.me")
  for source in "${sources[@]}"; do
    ip=$(curl -s6m5 -k "$source" 2>/dev/null | tr -d '\r\n' | grep -m1 -Eio '([0-9a-f]{0,4}:){2,7}[0-9a-f]{0,4}')
    if [[ -n $ip ]]; then
      echo "$ip"
      return 0
    fi
...
getIpv4Location(){
  local location
  location=$(curl -s4m5 -k https://myip.ipip.net 2>/dev/null | awk -F'来自于：' '{print $2}')
  if [[ -n $location ]]; then
    echo "$location"
    return 0
  fi
  location=$(curl -s4m5 -k https://ip.fm 2>/dev/null | sed -n 's/.*Location: //p')
  if [[ -n $location ]]; then
    echo "$location"
    return 0
  fi
...
getIpv6Location(){
  local location
  location=$(curl -s6m5 -k https://ip.fm 2>/dev/null | sed -n 's/.*Location: //p')
  if [[ -n $location ]]; then
    echo "$location"
    return 0
  fi
...
getSocksIpv4(){
  ...
  local sources=("https://api4.ipify.org?format=text" "https://checkip.amazonaws.com" "https://ipv4.icanhazip.com")
  for source in "${sources[@]}"; do
    ip=$(curl -sm3 --socks5 "localhost:$socksPort" "$source" 2>/dev/null | tr -d '\r\n' | grep -m1 -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}')
    if [[ -n $ip ]]; then
      echo "$ip"
      return 0
    fi
...
getSocksHostIp(){
  local socksPort=$1
  local ip
  local sources=("https://api64.ipify.org?format=text" "https://api.ipify.org?format=text" "https://icanhazip.com")
  for source in "${sources[@]}"; do
    ip=$(curl -sm3 -x "socks5h://localhost:$socksPort" "$source" 2>/dev/null | tr -d '\r\n' | grep -m1 -Eio '(([0-9]{1,3}\.){3}[0-9]{1,3}|([0-9a-f]{0,4}:){2,7}[0-9a-f]{0,4})')
    if [[ -n $ip ]]; then
      echo "$ip"
      return 0
    fi
...
v6(){
v4orv6(){
 hasIpv4=$(ip -4 addr show scope global up 2>/dev/null | grep -v '127.0.0.1')
 hasIpv6=$(ip -6 addr show scope global up 2>/dev/null | grep -v '::1')
 if [[ -z $hasIpv4 && -n $hasIpv6 ]]; then
 echo
 red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
 yellow "检测到纯IPV6 VPS，使用IPv6优先策略"
 ipv=prefer_ipv6
 else
 ipv=prefer_ipv4
 fi
...
sbport=${sbport:-'40000'}
resv1=$(getSocksIpv4 "$sbport")
resv2=$(getSocksHostIp "$sbport")
...
cp /root/sb.sh /usr/bin/sb 2>/dev/null
chmod +x /usr/bin/sb
}
...
resv1=$(getSocksIpv4 "$port")
resv2=$(getSocksHostIp "$port")
if [[ -z $resv1 && -z $resv2 ]]; then
