...
start_pre() {
  checkpath -d -m 0755 -o root:root /run/gost
}
EOF
...
function Install_ct() {
  ...
  if [[ ${addyn} == [Yy] ]]; then
    ...
    mkdir -p /etc/gost && wget --no-check-certificate https://gotunnel.oss-cn-shenzhen.aliyuncs.com/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  else
    rm -rf gost-linux-"$bit"-"$ct_new_ver".gz
    ...
    mv gost /usr/bin/gost
    chmod -R 777 /usr/bin/gost
    mkdir -p /etc/gost && wget --no-check-certificate https://raw.githubusercontent.com/KANIKIG/Multi-EasyGost/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
  fi

  check_file
  rc-update add gost default && rc-service gost restart
  echo "------------------------------"
...
cron_restart() {
  ...
  if [ "$numcron" == "1" ]; then
    ...
    if [ "$numcrontype" == "1" ]; then
      ...
      echo "0 0 */$cronhr * * * rc-service gost restart" >>/etc/crontab
      echo -e "定时重启设置成功！"
...
echo && echo -e "                 gost 一键安装配置脚本"${Red_font_prefix}[${shell_version}]${Font_color_suffix}"
  ----------- KANIKIG -----------
  特性: (1)本脚本采用OpenRC及gost配置文件对gost进行管理
